...
func CreateShardCoordinator(
	...
	if check.IfNil(nodesConfig) {
		return nil, "", errMoa.ErrNilGenesisNodesSetupHandler
	}
	if check.IfNil(pubKey) {
		return nil, "", errMoa.ErrNilPublicKey
	}
	if check.IfNil(log) {
		return nil, "", errMoa.ErrNilLogger
	}
...
func CreateNodesCoordinator(
	...
	if check.IfNil(nodeShufflerOut) {
		return nil, errMoa.ErrNilShuffleOutCloser
	}
	if check.IfNil(nodesConfig) {
		return nil, errMoa.ErrNilGenesisNodesSetupHandler
	}
	if check.IfNil(epochStartNotifier) {
		return nil, errMoa.ErrNilEpochStartNotifier
	}
	if check.IfNil(pubKey) {
		return nil, errMoa.ErrNilPublicKey
	}
	if check.IfNil(bootstrapParameters) {
		return nil, errMoa.ErrNilBootstrapParamsHandler
	}
	if chanNodeStop == nil {
