...
import (
	...
	"github.com/DharitriOne/drt-chain-go/debug"
	...
	"github.com/DharitriOne/drt-chain-go/node/disabled"
	...
	"github.com/DharitriOne/drt-chain-go/process/dataValidators"
	"github.com/DharitriOne/drt-chain-go/process/smartContract"
	procTx "github.com/DharitriOne/drt-chain-go/process/transaction"
...
func (n *Node) GetAllIssuedDCDTs(tokenType string, ctx context.Context) ([]string, error) {
	...
	for leaf := range chLeaves.LeavesChan {
		...
		if bytes.Equal(dcdtToken.TokenType, []byte(tokenType)) {
			tokens = append(tokens, tokenName)
		}
	}
