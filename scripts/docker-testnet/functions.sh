...
cloneRepositories() {
  ...
  git clone git@github.com:DharitriOne/drt-chain-deploy-go.git || true
  git clone git@github.com:DharitriOne/drt-chain-proxy-go.git || true
}
...
startSeedNode() {
  ...
  docker run -d --name seednode \
      -v ${TESTNETDIR}/seednode/config:/go/drt-chain-go/cmd/seednode/config \
  ...
  for ((i = 0; i < SHARDCOUNT; i++)); do
    for ((j = 0; j < SHARD_OBSERVERCOUNT; j++)); do
      ...
      docker run -d --name "observer${observerIdx}-${NETWORK_ADDRESS}.${IP_HOST_BYTE}-10200-shard${i}" \
          -v $TESTNETDIR/node/config:/go/drt-chain-go/cmd/node/config \
          --network ${DOCKER_NETWORK_NAME} \
          $publishPortArgs \
          node:dev \
