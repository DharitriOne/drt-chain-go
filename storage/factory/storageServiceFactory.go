...
import (
	...
	"github.com/DharitriOne/drt-chain-go/storage/clean"
	"github.com/DharitriOne/drt-chain-go/storage/databaseremover/disabled"
	"github.com/DharitriOne/drt-chain-go/storage/databaseremover/factory"
	storageDisabled "github.com/DharitriOne/drt-chain-go/storage/disabled"
...
func (psf *StorageServiceFactory) setUpDcdtSuppliesStorer(chainStorer *dataRetriever.ChainStorer, shardIDStr string) error {
	dcdtSuppliesUnit, err := psf.createDcdtSuppliesUnit(shardIDStr)
	if err != nil {
...
func (psf *StorageServiceFactory) createDcdtSuppliesUnit(shardIDStr string) (storage.Storer, error) {
	dcdtSuppliesConfig := psf.generalConfig.DbLookupExtensions.DCDTSuppliesStorageConfig
	dcdtSuppliesDbConfig := GetDBFromConfig(dcdtSuppliesConfig.DB)
	dcdtSuppliesDbConfig.FilePath = psf.pathManager.PathForStatic(shardIDStr, dcdtSuppliesConfig.DB.FilePath)
	dcdtSuppliesCacherConfig := GetCacherFromConfig(dcdtSuppliesConfig.Cache)

	dbConfigHandlerInstance := NewDBConfigHandler(dcdtSuppliesConfig.DB)
	dcdtSuppliesPersisterCreator, err := NewPersisterFactory(dbConfigHandlerInstance)
	if err != nil {
		return nil, err
	}

	return storageunit.NewStorageUnitFromConf(
		dcdtSuppliesCacherConfig, dcdtSuppliesDbConfig,
		dcdtSuppliesPersisterCreator)
}
