...
func testChainSimulatorSimpleStake(t *testing.T, targetEpoch int32, nodesStatus string) {
	...
	mintValue := big.NewInt(0).Mul(staking.OneREWA, big.NewInt(3000))
	...
	tx1Value := big.NewInt(0).Mul(big.NewInt(2499), staking.OneREWA)
	...
	tx3Value := big.NewInt(0).Mul(big.NewInt(2501), staking.OneREWA)
