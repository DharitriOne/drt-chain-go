...
func TestGetSkBytesFromP2pKey(t *testing.T) {
	...
	t.Run("invalid secret key should error", func(t *testing.T) {
		...
		invalidPem := `
-----BEGIN PRIVATE KEY for moa1ecdwux5tvanwryhr7cn5l9kc07ayquec7h2jc608kz0ycychzexslzxpfz-----
...
-----END PRIVATE KEY for moa1ecdwux5tvanwryhr7cn5l9kc07ayquec7h2jc608kz0ycychzexslzxpfz-----
`
		filePath := path.Join(t.TempDir(), "testFile01")
		file, err := os.Create(filePath)
		assert.Nil(t, err)
