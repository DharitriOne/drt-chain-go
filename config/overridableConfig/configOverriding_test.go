...
func TestOverrideConfigValues(t *testing.T) {
	...
	t.Run("should work for enableEpochs.toml", func(t *testing.T) {
		...
		err := OverrideConfigValues([]config.OverridableConfig{{Path: "EnableEpochs.DCDTMetadataContinuousCleanupEnableEpoch", Value: "37", File: "enableEpochs.toml"}}, configs)
		require.NoError(t, err)
	...
	t.Run("should work for enableRounds.toml", func(t *testing.T) {
		// TODO: fix this test
		t.Skip("skipped, as this test requires the fix from this PR: https://github.com/DharitriOne/drt-chain-go/pull/5851")
